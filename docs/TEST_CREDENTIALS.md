...
- None of the seeded vendors have an active subscription (`DemoDataSeeder` doesn't create `VendorSubscription` rows). Their existing seeded products still show up fine, but if you try adding a *new* product as `vendor1@dahashop.ng` etc. you'll hit the "Subscribe now" gate — go to Vendor > Subscription and pay to unlock it. Paystack works with a [test card](https://paystack.com/docs/payments/test-payments/) as soon as `PAYSTACK_SECRET_KEY`/`PAYSTACK_PUBLIC_KEY` are set; OPay won't work until a real, verified OPay Business merchant account provides `OPAY_MERCHANT_ID`/`OPAY_PUBLIC_KEY`/`OPAY_SECRET_KEY` (see `docs/DEVELOPER_TASKS.md`).
