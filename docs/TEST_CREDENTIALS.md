...
- None of the seeded vendors have an active subscription (`DemoDataSeeder` doesn't create `VendorSubscription` rows). Their existing seeded products still show up fine, but if you try adding a *new* product as `vendor1@dahashop.ng` etc. you'll hit the "Subscribe now" gate — go to Vendor > Subscription and pay with a [Paystack test card](https://paystack.com/docs/payments/test-payments/) to unlock it.
