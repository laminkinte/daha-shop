@if ($variant === 'bottom')
    <a href="{{ route('storefront.cart') }}" wire:navigate class="relative flex-1 flex flex-col items-center justify-center py-2 gap-0.5 {{ request()->routeIs('storefront.cart') ? 'text-green-700' : 'text-gray-500' }}">
        <span class="relative">
            ...
            @if ($count > 0)
                <span class="absolute -top-2 -right-2 bg-yellow-400 text-green-900 text-[10px] font-bold rounded-full h-4 w-4 flex items-center justify-center">
                    {{ $count }}
                </span>
            @endif
        </span>
