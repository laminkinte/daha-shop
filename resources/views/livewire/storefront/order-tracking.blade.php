    ...
    @foreach ($order->vendorOrders as $vendorOrder)
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <div class="flex items-center justify-between mb-4">
                ...
                @if (in_array($vendorOrder->status->value, ['rejected', 'failed', 'cancelled']))
                    <span class="text-xs font-semibold px-2 py-1 rounded-full bg-red-100 text-red-700 capitalize">{{ str_replace('_',' ',$vendorOrder->status->value) }}</span>
                @else
                    <span class="text-xs font-semibold px-2 py-1 rounded-full bg-green-100 text-green-700 capitalize">{{ str_replace('_',' ',$vendorOrder->status->value) }}</span>
                @endif
            </div>
                ...
                @foreach ($vendorOrder->items as $item)
                    ...
                    @if ($vendorOrder->status->value === 'delivered')
                        <div class="bg-gray-50 rounded-md p-3 mt-1">
                            @if ($item->review)
                                <div class="text-xs text-gray-500">Your review: <span class="text-yellow-500">{{ str_repeat('★', $item->review->rating) }}</span> {{ $item->review->comment }}</div>
                            @else
                                <div class="flex items-center gap-2">
                                    <select wire:model="ratings.{{ $item->id }}" class="text-xs rounded-md border-gray-300">
                                        @for ($i = 5; $i >= 1; $i--)
                                            <option value="{{ $i }}">{{ $i }} star{{ $i > 1 ? 's' : '' }}</option>
                                        @endfor
                                    </select>
                                    <input type="text" wire:model="comments.{{ $item->id }}" placeholder="Leave a review (optional)" class="flex-1 text-xs rounded-md border-gray-300">
                                    <button wire:click="submitReview({{ $item->id }})" class="text-xs bg-green-700 text-white px-3 py-1.5 rounded-md">Submit</button>
                                </div>
                            @endif
                        </div>
                    @endif
